document.addEventListener("DOMContentLoaded", () => {
  ...
  // Hardcoded for testing (replace with real userId/userRole after auth integration)
  const userId   = 1;
  ...
  /**
   * 1) Fetch the list of contracted stores from the backend
   */
  async function fetchStores() {
      ...
      try {
          ...
          // Expected response JSON: { success, code, message, data: [ { store_id, store_name, address, ... }, ... ] }
  ...
  /**
   * 2) Render store cards on the screen
   */
  ...
  function bindDeleteEvents() {
      scrollArea.querySelectorAll(".store-card .delete-btn")
          .forEach(btn => btn.addEventListener("click", async e => {
              e.stopPropagation();
              if (!confirm("Are you sure you want to remove this store from your contract list?")) return;
